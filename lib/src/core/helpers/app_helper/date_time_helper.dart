...
int relatedWeekday({int? month, int? year, int? day}) {
  var monthToString = addZeroPrefix(month);
  var dayToString = addZeroPrefix(day);
...
List dayToWeekday({int? month, int? year}) {
  final dayToWeekday = [];
  var dayCountForMon = month == 2 ? dayCountForFebruary(year) : dayCountForMonth[month! - 1];
  ...
  for (var i = 1; i <= dayCountForMon; i++) {
    dayToWeekday.add(i);
  }
  return dayToWeekday;
}
